...
def get_news_articles():
    urls = ["https://news.google.com/rss/headlines/section/topic/TECHNOLOGY?hl=en-US&gl=US&ceid=US:en"]
    ...
    for url in urls:
        try:
            ...
            for item in root.findall('.//item'):
                ...
                if title:
                    articles.append({"title": title, "description": clean_desc, "date": pub_date})
        except Exception as e:
...
def get_golden_keyword_and_source():
    ...
    if not articles:
        articles = [{"title": "Tech & Enterprise AI Major Industry Update", "description": "Key trends, metrics, and structural changes observed in the sector.", "date": "2026"}]

    headlines = [a["title"] for a in articles]
    prompt = f"""
    Review the following latest headlines in Tech & Enterprise AI:
    {json.dumps(headlines[:50])}
    
    Avoid previously covered topics:
    ...
    try:
        ...
        data = json.loads(res)
        kw = data.get("golden_keyword", "Tech & Enterprise AI Trends")
    except Exception as e:
        kw = "Tech & Enterprise AI Strategic Analysis"
